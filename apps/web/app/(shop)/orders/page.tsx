...
async function fetchOrders(page: number): Promise<{ data: OrderSummary[]; meta: { total: number; page: number; pageSize: number } }> {
  const res = await fetch(`${API}/api/orders?page=${page}&pageSize=10`, {
    credentials: 'include',
  });
  if (!res.ok) throw new Error('Error al cargar órdenes');
  return res.json() as Promise<{ data: OrderSummary[]; meta: { total: number; page: number; pageSize: number } }>;
}
...
export default function OrdersPage() {
  const [page, setPage] = [1, (_: number) => {}]; // simple single-page for MVP
