...
export class CartService {
  ...
  async updateItem(itemId: string, dto: UpdateCartItemDto, userId: string) {
    ...
    if (dto.qty > availableStock) throw new ConflictException(`Insufficient stock. Available: ${availableStock}`);
    const [updated] = await this.db
      .update(schema.cartItems)
      .set({ qty: dto.qty })
  ...
  async getAvailableStock(productId: string, variantId?: string): Promise<number> {
    if (variantId) {
      ...
      const reserved = await this.db
        .select({ total: sql<number>`COALESCE(SUM(${schema.inventoryReservations.qty}), 0)` })
